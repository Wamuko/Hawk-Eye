...
*/
    ...
    public void RenderClouds (RenderTexture tex)
	{
		...
		CustomGraphicsBlit(null, tex, mat, 0);
	}
	...
	public void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
        ...
        RenderTexture clouds = RenderTexture.GetTemporary(Screen.currentResolution.width, Screen.currentResolution.height, rtDepth, RenderTextureFormat.DefaultHDR);
		...
		if (EnviroSky.instance.cloudsMode == EnviroSky.EnviroCloudsMode.Volume || EnviroSky.instance.cloudsMode == EnviroSky.EnviroCloudsMode.Both) {
            RenderTexture tex = RenderTexture.GetTemporary(Screen.currentResolution.width / EnviroSky.instance.cloudsSettings.cloudsRenderResolution, Screen.currentResolution.height / EnviroSky.instance.cloudsSettings.cloudsRenderResolution, 0, RenderTextureFormat.DefaultHDR);
            if (!isAddionalCamera)
                EnviroSky.instance.cloudsRenderTarget = tex;
            RenderClouds (tex);
            //Blit clouds to final image
            blitMat.SetTexture ("_MainTex", source);
			blitMat.SetTexture ("_CloudsTex", tex);
