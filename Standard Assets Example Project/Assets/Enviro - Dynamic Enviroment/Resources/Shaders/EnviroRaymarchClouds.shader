	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 CalculateAmbientLighting(float altitude, float extinction_coeff) {
				...
				float3 isotropic_scattering_bottom = _CloudBaseColor.rgb * max(0.0, exp(ambient_term) - ambient_term * exponential_integral(ambient_term));
			...
			float CalculateCloudDensity(float3 pos, float height, float3 weather, bool details)
			{
				const float baseFreq = 1e-5;
				float4 coord = float4(pos * baseFreq * _BaseNoiseUV, 0.0);
				coord.xyz += float3(_CloudsAnimation.x*10, -_Time.x * 0.15 ,_CloudsAnimation.y * 10);
			...
			float3 GetDensityAlongRay(float3 pos, float3 PlanetCenter, float3 LightDirection, float3 weather,float h)
			{
				...
				float cloudDens = CalculateCloudDensity(pos, h, weather, true);
				...
				for (int i = 0; i < 5; i++)
				{
					...
					float cloudDensity = CalculateCloudDensity(samplePos, height, weather, true);
			...
			{						
				...
				{			
					...
					cloudDensity = CalculateCloudDensity(pos, height, weather, false);
					
					[branch]
					if (cloudDensity > 0.0)
					{
						float3 ds = GetDensityAlongRay(pos, pCent, LightDirection, weather, height);
						
						cloudDensity = ds.y;
						float currentOpticalDepth = cloudDensity * rayStepLength * _AlphaCoef * normalize(distance(pos, EyePosition));
